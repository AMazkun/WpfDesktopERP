...
namespace WpfApp5.Views.MainRoles
{
    ...
    /// </summary>
    public partial class DeveloperMain : Window
    {
        ...
        private void Show_Products(object sender, RoutedEventArgs e)
        {
            string title = "Pruducts";

            if (Check_Exists(title) == false)
            {
                DataManager.ResultStr = "Show Products";
                ClosableTab tabitem = new ClosableTab();
                tabitem.Title = title;
                mainTabControl.Items.Add(tabitem);
